    ...
    //  this.map.clearOverlays();
      this.deviceMapList.forEach((device, index) => {
        ...
        switch (device.product_type) {
          case 1:
            iconSize = new BMap.Size(10, 12);
            myIcon = new BMap.Icon(device.icon, iconSize, {
          ...
          case 2:
            if (isWeather == 1 && this.types.includes(4)) {
              iconSize = new BMap.Size(23, 22);
              myIcon = new BMap.Icon(device.icon, iconSize, {
                    anchor: new BMap.Size(12, 20),
          ...
          case 3:
            iconSize = new BMap.Size(40, 80);
            myIcon = new BMap.Icon(device.icon, iconSize, {
                anchor: new BMap.Size(20, 68),
                infoWindowAnchor: new BMap.Size(20, 40)
            });
            break;
          default:
             iconSize = new BMap.Size(11, 17);
       ...
       win.disableAutoPan();
        ...
        const clickFunc = function (e) {
          ...
          let indexNum=self.historyNum;
          self.historyNum=index;
          self.indexArr[curType - 1] = index;
          // if ((temp1 == 4 && temp2 != 4) || (temp1 != 4 && temp2 == 4)) {
          //   if(myCur==1){
          //      self.markers[index].marker.setIcon(
          ...
          // } else {
            ...
            //if(indexNum!==0){
                  ...
                  if (cur.product_type == 3) {
                    ...
                    iconSize = new BMap.Size(40, 80);
                    self.markers[indexNum].marker.setIcon(
                        ...
                        if(cur.status==2&&cur.is_open==1&&cur.is_weather==1){
                          icon = `${typeMap.get(4)}1`; 
                          iconSize = new BMap.Size(23, 22);
                          self.markers[indexNum].marker.setIcon(
                              new BMap.Icon(MapMark[icon], iconSize, {
                        ...
                        }else if(cur.status==2&&cur.is_open==2&&cur.is_weather==1){
                          icon = `${typeMap.get(4)}2`;
                          iconSize = new BMap.Size(23, 22);
                          self.markers[indexNum].marker.setIcon(
                              new BMap.Icon(MapMark[icon], iconSize, {
                        ...
                        }else if(cur.status==4&&cur.is_weather==1){
                          icon = `${typeMap.get(4)}3`;  
                          iconSize = new BMap.Size(23, 22);
                          self.markers[indexNum].marker.setIcon(
                              new BMap.Icon(MapMark[icon], iconSize, {
           ...
           //  }     
             let curIndex = self.markers[index];
             if(myCur==1){
               iconSize = new BMap.Size(10, 12);
                self.markers[index].marker.setIcon(
                  new BMap.Icon(MapMark[`${typeMap.get(temp2)}Selected`], iconSize, {
                 ...
                 if (curIndex.is_weather==1) {
                    iconSize = new BMap.Size(23, 22);
                      self.markers[index].marker.setIcon(
                        new BMap.Icon(MapMark[`${typeMap.get(4)}4`], iconSize, {
                        anchor: new BMap.Size(12, 20),
                        infoWindowAnchor: new BMap.Size(12, 0)
                      })
                    )

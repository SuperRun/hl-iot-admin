<template>
  <div class="weather-panel">
    ...
    <div class="operate flex jc-between">
      <div class="flex">
        <!-- <button type="button" class="btn btn-add" @click="add" v-if="false">
          添加
        </button>
    ...
    <!-- <el-form-item
          label="网关"
          :label-width="formLabelWidth"
          prop="gateway_product_id"
        >
          <el-select
            v-model="model.gateway_product_id"
            placeholder="请选择网关"
          >
            <el-option
              :key="gateway.id"
...
export default {
  ...
  data() {
    return {
      ...
      isLock: true,
      params: {
        product_type: 2, // 产品类型 有气象站的LED屏幕
        device_number: '', // 设备编号
        place_number: '', // 位号
        project_id: '', // 项目ID
        status: 0, // 状态
      ...
      deviceMap: {}, // 弹出框地图
      isWeather: true, // 用于判断地图显示weather图标
    };
  },
  methods: {
    weatherDetail(id) {
